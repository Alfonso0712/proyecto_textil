...
public class SesionFiltro implements Filter {
    ...
    private static final Object[][] RUTAS_PERMISOS = {
        ...
        { "/tiempos-reposo",    "PROD_REPOSO_VER"     }, // HU03: acceso base con VER; GESTION se verifica internamente
        { "/mermas",            "PROD_MERMA_VER"      }, // HU04: control fino de REG internamente
        { "/fallas-tela",       "PROD_FALLAS_VER"     }, // HU02: control fino de REG se verifica internamente
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        if (codigoRequerido != null) {
            ...
            String r = usuario.getNombreRol().toUpperCase();

            // ✅ FIX 1: EXCEPCIÓN PARA MAQUINISTAS EN /cargas-trabajo Y /defectos
            if ((rutaRelativa.startsWith("/cargas-trabajo") || rutaRelativa.startsWith("/defectos")) && usuario.getIdRol() == 6) {
                // Permitir acceso sin verificar permiso
            } 
            // ✅ FIX 2: EXCEPCIÓN PARA REPORTES (Roles Estratégicos y Operativos autorizados)
            else if (rutaRelativa.startsWith("/reportes") && (
                permisosUsuario.contains("RPT_MERMAS_CALIDAD") || 
                permisosUsuario.contains("RPT_DASHBOARD") || 
                r.contains("ADMIN") || r.contains("GERENTE") || r.contains("SUPERVISOR") || r.contains("JEFE_PRODUCCION") || r.contains("TIZADOR") || r.contains("JEFE_ALMACEN") || r.contains("MAQUINISTA")
            )) {
                // Permitir acceso
            }
            else {
