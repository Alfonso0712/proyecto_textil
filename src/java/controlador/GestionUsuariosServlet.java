...
public class GestionUsuariosServlet extends HttpServlet {
    ...
    private void listarUsuarios(HttpServletRequest req, HttpServletResponse resp)
        ...
        req.setAttribute("roles",    rolDAO.listarTodos()); // necesario para el modal flotante
    ...
    private void mostrarFormularioNuevo(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
        req.setAttribute("usuarios", usuarioDAO.listarTodos()); // para la tabla de fondo
    ...
    private void guardarNuevoUsuario(HttpServletRequest req, HttpServletResponse resp)
        ...
        u.setActivo(true);
        // Cambia esto para que lea el valor que envía el JS (true/false)
        String restringidoParam = req.getParameter("horarioRestringidoHidden");
        u.setHorarioRestringido("true".equals(restringidoParam));
        u.setHorarioDias(req.getParameter("horarioDias"));
        u.setHorarioInicio(req.getParameter("horarioInicio"));
        u.setHorarioFin(req.getParameter("horarioFin"));
    ...
    private void actualizarUsuario(HttpServletRequest req, HttpServletResponse resp)
        ...
        String restringidoParam = req.getParameter("horarioRestringidoHidden");
        u.setHorarioRestringido("true".equalsIgnoreCase(restringidoParam));
        u.setHorarioDias(req.getParameter("horarioDias"));
        u.setHorarioInicio(req.getParameter("horarioInicio"));
        u.setHorarioFin(req.getParameter("horarioFin"));
   ...
   private void cambiarEstado(HttpServletRequest req, HttpServletResponse resp,
        ...
        if (sesion != null && sesion.getIdUsuario() == id && !activar) {
            redirigirConMensaje(resp, req, "error", "No puedes desactivar tu propia cuenta.");
            return;
        }
        ...
        u.setActivo(activar);
        u.setPassword(null);          // ← No modificar la contraseña

        boolean ok = usuarioDAO.actualizar(u);
        String msg = ok ? (activar ? "Cuenta activada." : "Cuenta desactivada.") : "No se pudo cambiar el estado.";
    ...
    private void eliminarUsuario(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (sesion != null && sesion.getIdUsuario() == id) {
            redirigirConMensaje(resp, req, "error", "No puedes eliminar tu propia cuenta.");
            return;
        }

        // Verificar actividades
        if (usuarioDAO.tieneActividades(id)) {
            redirigirConMensaje(resp, req, "error", "El usuario tiene actividades registradas (OT, telas, etc.) y no puede ser eliminado.");
            return;
        }

        try {
            boolean ok = usuarioDAO.eliminar(id);
            redirigirConMensaje(resp, req,
                    ok ? "exito" : "error",
                    ok ? "Usuario eliminado definitivamente." : "No se pudo eliminar el usuario.");
        } catch (RuntimeException e) {
            redirigirConMensaje(resp, req, "error", e.getMessage());
        }
    }
    // ── HELPERS ───────────────────────────────────────────────
