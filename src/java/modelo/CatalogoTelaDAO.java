...
public class CatalogoTelaDAO {
    ...
    /**
    * Actualiza el flag 'requiere_reposo' en todas las telas que pertenecen a este catálogo.
    * @param idCatalogo  ID del catálogo modificado
    * @param nuevoValor  nuevo valor (true=requiere reposo, false=no requiere)
    */
    public void actualizarReposoEnTelas(int idCatalogo, boolean nuevoValor) {
        String sql = "UPDATE telas SET requiere_reposo = ? WHERE id_catalogo_tela = ?";
        try (Connection cn = ConexionDB.obtenerConexion();
             PreparedStatement ps = cn.prepareStatement(sql)) {
            ...
            throw new RuntimeException("Error al sincronizar reposo en telas: " + e.getMessage(), e);
        }
    }
}